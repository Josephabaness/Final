...
export default function Header() {
  ...
  return (
    <div className='bg-[linear-gradient(-180deg,#2ecc71,#27ae60)] pb-5 pt-2 text-white'>
      <div className='container'>
        <Navbar />
        <div className='mt-4 grid grid-cols-12 items-end gap-4'>
          <Link to='/' className='col-span-2'>
            <h1 className='text-2xl font-bold text-white ml-2'>Online Shop</h1>
          </Link>
          <form className='col-span-9' onSubmit={onSubmitSearch}>
            <div className='flex rounded-sm bg-white p-1'>
              <input
                ...
                {...register('name')}
              />
              <button className='flex-shrink-0 rounded-sm bg-green-500 py-2 px-6 hover:opacity-90'>
                <svg
            ...
            <div>
                    ...
                    {purchasesInCart && purchasesInCart.length > 0 ? (
                      <div className='p-2'>
                            ...
                            purchasesInCart.slice(0, MAX_PURCHASES).map((purchase: any) => (
                              <div className='mt-2 flex py-2 hover:bg-gray-100' key={purchase._id}>
                                ...
                                <div className='ml-2 flex-shrink-0'>
                                  <span className='text-green-500'>${formatCurrency(purchase.product.price)}</span>
                                </div>
                              </div>
                            ))
                          ) : (
                        ...
                        <div className='mt-6 flex items-center justify-between'>
                          ...
                          <Link
                            to={path.cart}
                            className='rounded-sm bg-green-500 px-4 py-2 capitalize text-white hover:bg-opacity-90'
                          >
              ...
              >
                <Link to={path.cart} className='relative'>
                  ...
                  {purchasesInCart && purchasesInCart.length > 0 && (
                    <span className='absolute top-[-5px] left-[17px] rounded-full bg-white px-[9px] py-[1px] text-xs text-green-500'>
